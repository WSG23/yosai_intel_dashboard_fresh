...
// NewGoBreaker constructs a gobreaker.CircuitBreaker instrumented with
// the breakerState metrics counter. Any existing OnStateChange callback in
// settings is preserved.
func NewGoBreaker(name string, settings gobreaker.Settings) *gobreaker.CircuitBreaker {
	if settings.Name == "" {
		settings.Name = name
	}
	...
	settings.OnStateChange = func(n string, from, to gobreaker.State) {
		...
		case gobreaker.StateClosed:
			breakerState.WithLabelValues(settings.Name, "closed").Inc()
		case gobreaker.StateOpen:
			breakerState.WithLabelValues(settings.Name, "open").Inc()
		case gobreaker.StateHalfOpen:
			breakerState.WithLabelValues(settings.Name, "half_open").Inc()
		}
	}
