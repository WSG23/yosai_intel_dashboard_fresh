...
class MappingService:
    ...
    def __init__(self, storage: StorageInterface, column_proc: ProcessorInterface, device_proc: ProcessorInterface) -> None:
        self.storage = storage
        self.column_proc = column_proc
        self.device_proc = device_proc

    def process_upload(self, df: pd.DataFrame, filename: str) -> Dict[str, Any]:
        try:
            column_result = self.column_proc.process(df, filename)
            device_result = self.device_proc.process(column_result["data"])
            return {"columns": column_result, "devices": device_result}
