...
class MVPTestApp(BaseDatabaseService):
            ...
            [  # type: ignore[attr-defined]
                    ...
                    [
                        dbc.Col(
                            [
                                ...
                                html.P(
                                    "Testing Complete Base Code Flow",
                ...
                dbc.Card(
                    [
                        ...
                        dbc.CardBody([html.Div(id="upload-results")]),
                    ],
                    className="mb-4",
                ),
                ...
                dbc.Card(
                    [
                        ...
                        dbc.CardBody([html.Div(id="column-mapping")]),
                    ],
                    className="mb-4",
                ),
                ...
                dbc.Card(
                    [
                        ...
                        dbc.CardBody([html.Div(id="device-analysis")]),
                    ],
                    className="mb-4",
                ),
                ...
                dbc.Card(
                    [
                        ...
                        dbc.CardBody([html.Div(id="enhanced-data")]),
                    ],
                    className="mb-4",
                ),
                # Export Section
    ...
    def _register_callbacks(self):
        ...
        @self.app.callback(
            ...
            [State("upload-area", "filename"), State("session-store", "data")],
        )
            ...
            try:
                ...
                if filename in file_info:
                    ...
                    try:
                        ...
                        logger.info(
                            f"📊 Retrieved DataFrame: {df.shape if df is not None else 'None'}"
                        )
                    except Exception as e:
                ...
                upload_display = self._create_upload_display(
                    upload_results, file_info.get(filename, {})
                )
                ...
                error_display = dbc.Alert(
                    f"Processing failed: {safe_str(e)}", color="danger"
                )
                return error_display, "", "", "", session_data

        @self.app.callback(
            Output("download-component", "data"),
            [Input("download-csv", "n_clicks"), Input("download-json", "n_clicks")],
        ...
        def handle_download(csv_clicks, json_clicks, session_data):
            ...
            if button_id == "download-csv":
                return dcc.send_data_frame(
                    df.to_csv, f"enhanced_{filename}.csv", index=False
                )
            elif button_id == "download-json":
                return dcc.send_data_frame(
                    df.to_json, f"enhanced_{filename}.json", orient="records"
                )
    ...
    def _create_device_display(self, df, filename):
        ...
        try:
            ...
            if user_mappings:
                ...
                assert self.upload_service is not None
                learned_applied = self.upload_service.auto_apply_learned_mappings(
                    df, filename
                )
            ...
            status_alert = dbc.Alert(
                (
                    f"📋 {len(store_data)} devices ready for mapping"
                    if store_data
            ...
            return html.Div(
                [html.H6("🚪 Device Configuration:"), status_alert, device_section]
            )
