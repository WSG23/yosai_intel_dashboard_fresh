        ...
        data.append(
            {
                ...
                "entry_without_badge": random.choice(
                    [True, False, False, False]
                ),  # 25% no badge
                "device_status": random.choice(
                    ["normal", "normal", "normal", "error"]
                ),  # 25% error
                "location": random.choice(["Building A", "Building B", "Building C"]),
                "department": random.choice(
                    ["Engineering", "Sales", "HR", "Marketing", "Operations"]
                ),
            }
        )
    ...
    try:
        ...
        try:
            ...
            if step1_rows != results["input_rows"]:
                results["errors"].append(
                    f"Step 1: Row count mismatch - expected {results['input_rows']}, got {step1_rows}"
                )
        ...
        try:
            ...
            processor_result = processor.process_file(
                open(csv_path, "rb").read(), os.path.basename(csv_path)
            )

            if processor_result.get("success"):
                step2_rows = processor_result.get("rows", 0)
                results["steps_completed"].append(
                    f"Step 2 - FileProcessor: {step2_rows:,} rows"
                )
                logger.info(f"✅ Step 2: FileProcessor processed {step2_rows:,} rows")

                if step2_rows != step1_rows:
                    results["errors"].append(
                        f"Step 2: Row count changed - was {step1_rows}, now {step2_rows}"
                    )
            else:
                results["errors"].append(
                    f"Step 2: FileProcessor failed - {processor_result.get('error')}"
                )

        except ImportError:
            results["steps_completed"].append(
                "Step 2 - FileProcessor: SKIPPED (not available)"
            )
        ...
        try:
            ...
            validated_df, needs_chunking = validator.validate_for_analysis(
                df_raw.copy()
            )
            step3_rows = len(validated_df)

            results["steps_completed"].append(
                f"Step 3 - Validator: {step3_rows:,} rows (chunking: {needs_chunking})"
            )
            logger.info(
                f"✅ Step 3: Validator processed {step3_rows:,} rows, chunking: {needs_chunking}"
            )

            if step3_rows < step1_rows * 0.95:  # More than 5% data loss
                results["errors"].append(
                    f"Step 3: Significant data loss - {step1_rows - step3_rows} rows removed"
                )

        except ImportError:
            results["steps_completed"].append(
                "Step 3 - Validator: SKIPPED (not available)"
            )
        ...
        try:
            ...
            step4_rows = result.get("processing_summary", {}).get(
                "rows_processed", "UNKNOWN"
            )

            results["steps_completed"].append(
                f"Step 4 - Analytics: {step4_rows} rows processed"
            )
            logger.info(f"✅ Step 4: Analytics processed {step4_rows} rows")

            results["final_row_count"] = (
                step4_rows if isinstance(step4_rows, int) else len(test_df)
            )

        except ImportError:
            results["steps_completed"].append(
                "Step 4 - Analytics: SKIPPED (not available)"
            )
        ...
        if hasattr(dynamic_config, "analytics"):
            ...
            if hasattr(analytics, "force_full_dataset_analysis"):
                if not analytics.force_full_dataset_analysis:
                    config_issues.append(
                        "analytics.force_full_dataset_analysis is False"
                    )
            else:
            ...
            if hasattr(analytics, "max_records_per_query"):
                if analytics.max_records_per_query < 100000:
                    config_issues.append(
                        f"analytics.max_records_per_query too low: {analytics.max_records_per_query}"
                    )

            if hasattr(analytics, "batch_size"):
                if analytics.batch_size < 1000:
                    config_issues.append(
                        f"analytics.batch_size too low: {analytics.batch_size}"
                    )
        else:
        ...
        if hasattr(dynamic_config, "security"):
            ...
            if hasattr(security, "max_upload_mb"):
                if security.max_upload_mb < 50:
                    config_issues.append(
                        f"security.max_upload_mb too low: {security.max_upload_mb}"
                    )
...
def main():
    ...
    # Final assessment
    success_rate = (
        results["final_row_count"] / results["input_rows"]
        if results["final_row_count"]
        else 0
    )
    ...
    if success_rate >= 0.99:  # 99% or more rows processed
        ...
        logger.warning(
            f"⚠️  PARTIAL SUCCESS: {success_rate:.1%} of data processed (some data loss)"
        )
    else:
