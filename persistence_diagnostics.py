        ...
        for i, file_path in enumerate(mapping_files):
            ...
            try:
                with open(file_path, "r", encoding="utf-8") as f:
                    ...
                    mappings = data.get("device_mappings", {}) or data.get(
                        "mappings", {}
                    )
                    if mappings:
...
def test_save_load_cycle():
    ...
    try:
        ...
        test_filename = (
            f"diagnostic_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
        ...
        save_result = learning_service.save_user_device_mappings(
            test_df, test_filename, test_mappings
        )
        ...
        if save_result:
            ...
            if our_file:
                ...
                with open(our_file, "r", encoding="utf-8") as f:
                    ...
                    mappings = file_data.get("device_mappings", {}) or file_data.get(
                        "mappings", {}
                    )
                    print(f"   - Mappings found: {len(mappings)}")
                    for device, mapping in mappings.items():
                        print(
                            f"     * {device}: floor={mapping.get('floor_number')}, security={mapping.get('security_level')}"
                        )
            ...
            retrieved_mappings = learning_service.get_user_device_mappings(
                test_filename
            )
            ...
            if retrieved_mappings:
                ...
                for device, mapping in retrieved_mappings.items():
                    print(
                        f"   - {device}: {mapping.get('device_name')} (floor {mapping.get('floor_number')})"
                    )
                ...
                if len(retrieved_mappings) == len(test_mappings):
                    ...
                    print(
                        f"❌ Device count mismatch: saved {len(test_mappings)}, retrieved {len(retrieved_mappings)}"
                    )
                ...
                if "test_lobby" in retrieved_mappings:
                    ...
                    if (
                        lobby_data.get("floor_number") == 1
                        and lobby_data.get("security_level") == 3
                    ):
    ...
    try:
        ...
        if learning_service.learned_mappings:
            for fingerprint, data in learning_service.learned_mappings.items():
                ...
                print(
                    f"   - {fingerprint[:8]}: {filename} ({device_count} devices, saved: {saved_at})"
                )
        else:
