                    ...
                    if suggestions:
                        for col, suggestion in suggestions.items():
                            ...
                            if field and confidence > 0:
                                st.write(
                                    f"• **{col}** → `{field}` (confidence: {confidence:.1f})"
                                )
                    else:
...
def show_device_mapping():
    ...
    try:
        ...
        selected_mapping = st.selectbox(
            "Select Mapping to View:", list(mapping_options.keys())
        )
            ...
            if device_mappings:
                ...
                with col1:
                    floor_filter = st.multiselect(
                        "Filter by Floor:", sorted(device_df["Floor"].unique())
                    )
                with col2:
                    security_filter = st.multiselect(
                        "Filter by Security Level:",
                        sorted(device_df["Security Level"].unique()),
                    )
                ...
                if security_filter:
                    filtered_df = filtered_df[
                        filtered_df["Security Level"].isin(security_filter)
                    ]
                    ...
                    if st.button(f"Run {test_name}", key=test_name):
                        try:
                            ...
                            if isinstance(result, pd.DataFrame):
                                st.write(
                                    f"**Result**: DataFrame with {len(result)} rows"
                                )
