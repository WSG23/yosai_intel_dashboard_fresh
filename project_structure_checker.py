...
def check_project_structure(path="."):
    ...
    print(f"\n📋 Project Files:")
    common_files = ['requirements.txt', 'setup.py', 'README.md', 'Dockerfile', 
    ...
    if not (project_path / '__init__.py').exists() and not (project_path / 'setup.py').exists():
        issues.append("No __init__.py or setup.py found in root - may not be a Python package")
    
    git_dir = project_path / '.git'
    if not git_dir.exists():
        issues.append("No .git directory found - project may not be under version control")
...
if __name__ == "__main__":
    ...
    if has_python:
        ...
        print(f"   Run: python3 code_analyzer.py {Path(path).absolute()}")
    else:
