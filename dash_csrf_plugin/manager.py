...
class CSRFManager:
    ...
    def __init__(self, app: dash.Dash, config: CSRFConfig, mode: CSRFMode):
        """
    ...
    def _init_testing_mode(self) -> None:
        ...
        server = self.app.server
        server.config.update({"WTF_CSRF_ENABLED": False, "TESTING": True})
            ...
            if self.csrf_protect:
                ...
                if view_func:
                    try:
                        ...
                        logger.warning(f"Could not exempt view for route {route}: {e}")
                else:
                    logger.debug(
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "server_csrf_enabled": self.app.server.config.get(
    ...
    def is_enabled(self) -> bool:
        ...
        return (
            ...
            and self.mode in [CSRFMode.ENABLED, CSRFMode.PRODUCTION]
        )

    def health_check(self) -> Dict[str, Any]:
        """Perform health check on CSRF system"""
        status = {"healthy": True, "issues": [], "recommendations": []}
        ...
        if secret_key in ["dev", "development", "test", "secret"]:
            ...
            status["recommendations"].append(
                "Use a cryptographically secure secret key"
            )
        ...
        if self.mode == CSRFMode.PRODUCTION:
            if not self.config.ssl_strict:
                status["recommendations"].append(
                    "Enable SSL strict mode for production"
                )
