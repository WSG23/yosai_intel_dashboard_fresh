...
class CSRFConfig:
    ...
    def for_production(cls, secret_key: str, **kwargs):
        ...
        defaults = {"enabled": True, "secret_key": secret_key, "ssl_strict": True}
        defaults.update(kwargs)
        return cls(**defaults)
    ...
    def for_testing(cls, **kwargs):
        ...
        defaults = {"enabled": False, "secret_key": "test-secret-key"}
        defaults.update(kwargs)
        return cls(**defaults)
...
class EnhancedCSRFManager:
            ...
            if self.csrf_protect:
                ...
                if view:
                    try:
                        ...
                        logger.debug(
                            f"Successfully exempted view function {view_function}"
                        )
                    except Exception as e:
                        logger.warning(
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "flask_csrf_enabled": (
                self.app.server.config.get("WTF_CSRF_ENABLED", False)
                if self.app
                else False
            ),
...
def setup_enhanced_csrf_protection(
    app: dash.Dash, mode: CSRFMode = CSRFMode.AUTO
) -> DashCSRFPlugin:
