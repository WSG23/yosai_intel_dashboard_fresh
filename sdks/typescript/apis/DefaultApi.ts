/* tslint:disable */
/* eslint-disable */
/**
 * Yōsai Intel Dashboard API
 * Physical security intelligence and access control API.
 *
 * The version of the OpenAPI document: 1.0.0
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import * as runtime from '../runtime';
import type { GetCsrfToken200Response } from '../models/index';
import {
  GetCsrfToken200ResponseFromJSON,
  GetCsrfToken200ResponseToJSON,
} from '../models/index';

export interface DeleteFeatureFlagRequest {
  name: string;
}

export interface GetChartDataRequest {
  chartType: string;
}

export interface GetExplanationRequest {
  predictionId: string;
}

export interface GetFeatureFlagRequest {
  name: string;
}

export interface GetFeatureFlagAuditRequest {
  name: string;
}

export interface GetModelMonitoringEventsRequest {
  modelName: string;
  start?: Date;
  end?: Date;
}

export interface UpdateFeatureFlagRequest {
  name: string;
}

/**
 *
 */
export class DefaultApi extends runtime.BaseAPI {
  /**
   * Analytics service health
   */
  async analyticsHealthRaw(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    if (this.configuration && this.configuration.apiKey) {
      headerParameters['X-CSRF-Token'] =
        await this.configuration.apiKey('X-CSRF-Token'); // csrfToken authentication
    }

    if (this.configuration && this.configuration.accessToken) {
      const token = this.configuration.accessToken;
      const tokenString = await token('jwtAuth', []);

      if (tokenString) {
        headerParameters['Authorization'] = `Bearer ${tokenString}`;
      }
    }

    let urlPath = `/analytics/health`;

    const response = await this.request(
      {
        path: urlPath,
        method: 'GET',
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides,
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   * Analytics service health
   */
  async analyticsHealth(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<void> {
    await this.analyticsHealthRaw(initOverrides);
  }

  /**
   * Create feature flag
   */
  async createFeatureFlagRaw(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    if (this.configuration && this.configuration.apiKey) {
      headerParameters['X-CSRF-Token'] =
        await this.configuration.apiKey('X-CSRF-Token'); // csrfToken authentication
    }

    if (this.configuration && this.configuration.accessToken) {
      const token = this.configuration.accessToken;
      const tokenString = await token('jwtAuth', []);

      if (tokenString) {
        headerParameters['Authorization'] = `Bearer ${tokenString}`;
      }
    }

    let urlPath = `/feature-flags`;

    const response = await this.request(
      {
        path: urlPath,
        method: 'POST',
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides,
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   * Create feature flag
   */
  async createFeatureFlag(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<void> {
    await this.createFeatureFlagRaw(initOverrides);
  }

  /**
   * Delete feature flag
   */
  async deleteFeatureFlagRaw(
    requestParameters: DeleteFeatureFlagRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    if (requestParameters['name'] == null) {
      throw new runtime.RequiredError(
        'name',
        'Required parameter "name" was null or undefined when calling deleteFeatureFlag().',
      );
    }

    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    if (this.configuration && this.configuration.apiKey) {
      headerParameters['X-CSRF-Token'] =
        await this.configuration.apiKey('X-CSRF-Token'); // csrfToken authentication
    }

    if (this.configuration && this.configuration.accessToken) {
      const token = this.configuration.accessToken;
      const tokenString = await token('jwtAuth', []);

      if (tokenString) {
        headerParameters['Authorization'] = `Bearer ${tokenString}`;
      }
    }

    let urlPath = `/feature-flags/{name}`;
    urlPath = urlPath.replace(
      `{${'name'}}`,
      encodeURIComponent(String(requestParameters['name'])),
    );

    const response = await this.request(
      {
        path: urlPath,
        method: 'DELETE',
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides,
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   * Delete feature flag
   */
  async deleteFeatureFlag(
    requestParameters: DeleteFeatureFlagRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<void> {
    await this.deleteFeatureFlagRaw(requestParameters, initOverrides);
  }

  /**
   * Retrieve chart data
   */
  async getChartDataRaw(
    requestParameters: GetChartDataRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    if (requestParameters['chartType'] == null) {
      throw new runtime.RequiredError(
        'chartType',
        'Required parameter "chartType" was null or undefined when calling getChartData().',
      );
    }

    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    if (this.configuration && this.configuration.apiKey) {
      headerParameters['X-CSRF-Token'] =
        await this.configuration.apiKey('X-CSRF-Token'); // csrfToken authentication
    }

    if (this.configuration && this.configuration.accessToken) {
      const token = this.configuration.accessToken;
      const tokenString = await token('jwtAuth', []);

      if (tokenString) {
        headerParameters['Authorization'] = `Bearer ${tokenString}`;
      }
    }

    let urlPath = `/analytics/chart/{chart_type}`;
    urlPath = urlPath.replace(
      `{${'chart_type'}}`,
      encodeURIComponent(String(requestParameters['chartType'])),
    );

    const response = await this.request(
      {
        path: urlPath,
        method: 'GET',
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides,
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   * Retrieve chart data
   */
  async getChartData(
    requestParameters: GetChartDataRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<void> {
    await this.getChartDataRaw(requestParameters, initOverrides);
  }

  /**
   * Get CSRF token
   */
  async getCsrfTokenRaw(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<GetCsrfToken200Response>> {
    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    if (this.configuration && this.configuration.apiKey) {
      headerParameters['X-CSRF-Token'] =
        await this.configuration.apiKey('X-CSRF-Token'); // csrfToken authentication
    }

    if (this.configuration && this.configuration.accessToken) {
      const token = this.configuration.accessToken;
      const tokenString = await token('jwtAuth', []);

      if (tokenString) {
        headerParameters['Authorization'] = `Bearer ${tokenString}`;
      }
    }

    let urlPath = `/csrf-token`;

    const response = await this.request(
      {
        path: urlPath,
        method: 'GET',
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides,
    );

    return new runtime.JSONApiResponse(response, (jsonValue) =>
      GetCsrfToken200ResponseFromJSON(jsonValue),
    );
  }

  /**
   * Get CSRF token
   */
  async getCsrfToken(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<GetCsrfToken200Response> {
    const response = await this.getCsrfTokenRaw(initOverrides);
    return await response.value();
  }

  /**
   * Get prediction explanation
   */
  async getExplanationRaw(
    requestParameters: GetExplanationRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    if (requestParameters['predictionId'] == null) {
      throw new runtime.RequiredError(
        'predictionId',
        'Required parameter "predictionId" was null or undefined when calling getExplanation().',
      );
    }

    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    if (this.configuration && this.configuration.apiKey) {
      headerParameters['X-CSRF-Token'] =
        await this.configuration.apiKey('X-CSRF-Token'); // csrfToken authentication
    }

    if (this.configuration && this.configuration.accessToken) {
      const token = this.configuration.accessToken;
      const tokenString = await token('jwtAuth', []);

      if (tokenString) {
        headerParameters['Authorization'] = `Bearer ${tokenString}`;
      }
    }

    let urlPath = `/explanations/{prediction_id}`;
    urlPath = urlPath.replace(
      `{${'prediction_id'}}`,
      encodeURIComponent(String(requestParameters['predictionId'])),
    );

    const response = await this.request(
      {
        path: urlPath,
        method: 'GET',
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides,
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   * Get prediction explanation
   */
  async getExplanation(
    requestParameters: GetExplanationRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<void> {
    await this.getExplanationRaw(requestParameters, initOverrides);
  }

  /**
   * Get feature flag
   */
  async getFeatureFlagRaw(
    requestParameters: GetFeatureFlagRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    if (requestParameters['name'] == null) {
      throw new runtime.RequiredError(
        'name',
        'Required parameter "name" was null or undefined when calling getFeatureFlag().',
      );
    }

    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    if (this.configuration && this.configuration.apiKey) {
      headerParameters['X-CSRF-Token'] =
        await this.configuration.apiKey('X-CSRF-Token'); // csrfToken authentication
    }

    if (this.configuration && this.configuration.accessToken) {
      const token = this.configuration.accessToken;
      const tokenString = await token('jwtAuth', []);

      if (tokenString) {
        headerParameters['Authorization'] = `Bearer ${tokenString}`;
      }
    }

    let urlPath = `/feature-flags/{name}`;
    urlPath = urlPath.replace(
      `{${'name'}}`,
      encodeURIComponent(String(requestParameters['name'])),
    );

    const response = await this.request(
      {
        path: urlPath,
        method: 'GET',
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides,
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   * Get feature flag
   */
  async getFeatureFlag(
    requestParameters: GetFeatureFlagRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<void> {
    await this.getFeatureFlagRaw(requestParameters, initOverrides);
  }

  /**
   * Get feature flag audit history
   */
  async getFeatureFlagAuditRaw(
    requestParameters: GetFeatureFlagAuditRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    if (requestParameters['name'] == null) {
      throw new runtime.RequiredError(
        'name',
        'Required parameter "name" was null or undefined when calling getFeatureFlagAudit().',
      );
    }

    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    if (this.configuration && this.configuration.apiKey) {
      headerParameters['X-CSRF-Token'] =
        await this.configuration.apiKey('X-CSRF-Token'); // csrfToken authentication
    }

    if (this.configuration && this.configuration.accessToken) {
      const token = this.configuration.accessToken;
      const tokenString = await token('jwtAuth', []);

      if (tokenString) {
        headerParameters['Authorization'] = `Bearer ${tokenString}`;
      }
    }

    let urlPath = `/feature-flags/{name}/audit`;
    urlPath = urlPath.replace(
      `{${'name'}}`,
      encodeURIComponent(String(requestParameters['name'])),
    );

    const response = await this.request(
      {
        path: urlPath,
        method: 'GET',
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides,
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   * Get feature flag audit history
   */
  async getFeatureFlagAudit(
    requestParameters: GetFeatureFlagAuditRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<void> {
    await this.getFeatureFlagAuditRaw(requestParameters, initOverrides);
  }

  /**
   * Get model monitoring events
   */
  async getModelMonitoringEventsRaw(
    requestParameters: GetModelMonitoringEventsRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    if (requestParameters['modelName'] == null) {
      throw new runtime.RequiredError(
        'modelName',
        'Required parameter "modelName" was null or undefined when calling getModelMonitoringEvents().',
      );
    }

    const queryParameters: any = {};

    if (requestParameters['start'] != null) {
      queryParameters['start'] = (
        requestParameters['start'] as any
      ).toISOString();
    }

    if (requestParameters['end'] != null) {
      queryParameters['end'] = (requestParameters['end'] as any).toISOString();
    }

    const headerParameters: runtime.HTTPHeaders = {};

    if (this.configuration && this.configuration.apiKey) {
      headerParameters['X-CSRF-Token'] =
        await this.configuration.apiKey('X-CSRF-Token'); // csrfToken authentication
    }

    if (this.configuration && this.configuration.accessToken) {
      const token = this.configuration.accessToken;
      const tokenString = await token('jwtAuth', []);

      if (tokenString) {
        headerParameters['Authorization'] = `Bearer ${tokenString}`;
      }
    }

    let urlPath = `/model-monitoring/{model_name}`;
    urlPath = urlPath.replace(
      `{${'model_name'}}`,
      encodeURIComponent(String(requestParameters['modelName'])),
    );

    const response = await this.request(
      {
        path: urlPath,
        method: 'GET',
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides,
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   * Get model monitoring events
   */
  async getModelMonitoringEvents(
    requestParameters: GetModelMonitoringEventsRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<void> {
    await this.getModelMonitoringEventsRaw(requestParameters, initOverrides);
  }

  /**
   * Summarize access patterns
   */
  async getPatternsRaw(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    if (this.configuration && this.configuration.apiKey) {
      headerParameters['X-CSRF-Token'] =
        await this.configuration.apiKey('X-CSRF-Token'); // csrfToken authentication
    }

    if (this.configuration && this.configuration.accessToken) {
      const token = this.configuration.accessToken;
      const tokenString = await token('jwtAuth', []);

      if (tokenString) {
        headerParameters['Authorization'] = `Bearer ${tokenString}`;
      }
    }

    let urlPath = `/analytics/patterns`;

    const response = await this.request(
      {
        path: urlPath,
        method: 'GET',
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides,
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   * Summarize access patterns
   */
  async getPatterns(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<void> {
    await this.getPatternsRaw(initOverrides);
  }

  /**
   * Get user settings
   */
  async getSettingsRaw(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    if (this.configuration && this.configuration.apiKey) {
      headerParameters['X-CSRF-Token'] =
        await this.configuration.apiKey('X-CSRF-Token'); // csrfToken authentication
    }

    if (this.configuration && this.configuration.accessToken) {
      const token = this.configuration.accessToken;
      const tokenString = await token('jwtAuth', []);

      if (tokenString) {
        headerParameters['Authorization'] = `Bearer ${tokenString}`;
      }
    }

    let urlPath = `/settings`;

    const response = await this.request(
      {
        path: urlPath,
        method: 'GET',
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides,
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   * Get user settings
   */
  async getSettings(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<void> {
    await this.getSettingsRaw(initOverrides);
  }

  /**
   * List data sources
   */
  async getSourcesRaw(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    if (this.configuration && this.configuration.apiKey) {
      headerParameters['X-CSRF-Token'] =
        await this.configuration.apiKey('X-CSRF-Token'); // csrfToken authentication
    }

    if (this.configuration && this.configuration.accessToken) {
      const token = this.configuration.accessToken;
      const tokenString = await token('jwtAuth', []);

      if (tokenString) {
        headerParameters['Authorization'] = `Bearer ${tokenString}`;
      }
    }

    let urlPath = `/analytics/sources`;

    const response = await this.request(
      {
        path: urlPath,
        method: 'GET',
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides,
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   * List data sources
   */
  async getSources(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<void> {
    await this.getSourcesRaw(initOverrides);
  }

  /**
   * List feature flags
   */
  async listFeatureFlagsRaw(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    if (this.configuration && this.configuration.apiKey) {
      headerParameters['X-CSRF-Token'] =
        await this.configuration.apiKey('X-CSRF-Token'); // csrfToken authentication
    }

    if (this.configuration && this.configuration.accessToken) {
      const token = this.configuration.accessToken;
      const tokenString = await token('jwtAuth', []);

      if (tokenString) {
        headerParameters['Authorization'] = `Bearer ${tokenString}`;
      }
    }

    let urlPath = `/feature-flags`;

    const response = await this.request(
      {
        path: urlPath,
        method: 'GET',
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides,
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   * List feature flags
   */
  async listFeatureFlags(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<void> {
    await this.listFeatureFlagsRaw(initOverrides);
  }

  /**
   * Refresh access token
   */
  async refreshTokenRaw(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    if (this.configuration && this.configuration.apiKey) {
      headerParameters['X-CSRF-Token'] =
        await this.configuration.apiKey('X-CSRF-Token'); // csrfToken authentication
    }

    if (this.configuration && this.configuration.accessToken) {
      const token = this.configuration.accessToken;
      const tokenString = await token('jwtAuth', []);

      if (tokenString) {
        headerParameters['Authorization'] = `Bearer ${tokenString}`;
      }
    }

    let urlPath = `/token/refresh`;

    const response = await this.request(
      {
        path: urlPath,
        method: 'POST',
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides,
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   * Refresh access token
   */
  async refreshToken(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<void> {
    await this.refreshTokenRaw(initOverrides);
  }

  /**
   * Update feature flag
   */
  async updateFeatureFlagRaw(
    requestParameters: UpdateFeatureFlagRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    if (requestParameters['name'] == null) {
      throw new runtime.RequiredError(
        'name',
        'Required parameter "name" was null or undefined when calling updateFeatureFlag().',
      );
    }

    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    if (this.configuration && this.configuration.apiKey) {
      headerParameters['X-CSRF-Token'] =
        await this.configuration.apiKey('X-CSRF-Token'); // csrfToken authentication
    }

    if (this.configuration && this.configuration.accessToken) {
      const token = this.configuration.accessToken;
      const tokenString = await token('jwtAuth', []);

      if (tokenString) {
        headerParameters['Authorization'] = `Bearer ${tokenString}`;
      }
    }

    let urlPath = `/feature-flags/{name}`;
    urlPath = urlPath.replace(
      `{${'name'}}`,
      encodeURIComponent(String(requestParameters['name'])),
    );

    const response = await this.request(
      {
        path: urlPath,
        method: 'PUT',
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides,
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   * Update feature flag
   */
  async updateFeatureFlag(
    requestParameters: UpdateFeatureFlagRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<void> {
    await this.updateFeatureFlagRaw(requestParameters, initOverrides);
  }

  /**
   * Update user settings
   */
  async updateSettingsRaw(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    if (this.configuration && this.configuration.apiKey) {
      headerParameters['X-CSRF-Token'] =
        await this.configuration.apiKey('X-CSRF-Token'); // csrfToken authentication
    }

    if (this.configuration && this.configuration.accessToken) {
      const token = this.configuration.accessToken;
      const tokenString = await token('jwtAuth', []);

      if (tokenString) {
        headerParameters['Authorization'] = `Bearer ${tokenString}`;
      }
    }

    let urlPath = `/settings`;

    const response = await this.request(
      {
        path: urlPath,
        method: 'POST',
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides,
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   * Update user settings
   */
  async updateSettings(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<void> {
    await this.updateSettingsRaw(initOverrides);
  }

  /**
   * Upload a file
   */
  async uploadFileRaw(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    if (this.configuration && this.configuration.apiKey) {
      headerParameters['X-CSRF-Token'] =
        await this.configuration.apiKey('X-CSRF-Token'); // csrfToken authentication
    }

    if (this.configuration && this.configuration.accessToken) {
      const token = this.configuration.accessToken;
      const tokenString = await token('jwtAuth', []);

      if (tokenString) {
        headerParameters['Authorization'] = `Bearer ${tokenString}`;
      }
    }

    let urlPath = `/upload`;

    const response = await this.request(
      {
        path: urlPath,
        method: 'POST',
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides,
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   * Upload a file
   */
  async uploadFile(
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<void> {
    await this.uploadFileRaw(initOverrides);
  }
}
