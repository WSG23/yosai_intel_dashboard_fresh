...
class YourSystemCallbackAuditor:
    ...
    def scan_complete_codebase(self) -> Dict[str, Any]:
        ...
        for py_file in python_files:
            ...
            for pattern in file_patterns:
                ...
                results['patterns_by_type'][
                    pattern.pattern_type
                ].append(pattern)

                # Track namespace usage
                if pattern.component_name:
                    self.namespace_usage[pattern.component_name].append(
                        pattern.callback_id
                    )
        ...
        print(
            f"✅ Audit complete! Found {len(self.patterns)} callbacks across"
            f" {len(self.pattern_counts)} pattern types"
        )
    ...
    def _get_decorator_name(self, decorator: ast.AST) -> str:
        ...
        if isinstance(decorator, ast.Name):
            ...
            return (
                f"{self._get_decorator_name(decorator.value)}." f"{decorator.attr}"
            )
        ...
        for output_id, usages in self.output_conflicts.items():
            if len(usages) > 1:
                ...
                conflict = ConflictAnalysis(
                    ...
                    conflicting_callbacks=[
                        usage['callback_id']
                        for usage in usages
                        if usage['callback_id']
                    ],
                    severity=severity,
                    resolution_suggestion=
                        self._suggest_conflict_resolution(output_id, usages)
                )
    ...
    def _assess_conflict_severity(self, output_id: str, usages: List[dict]) -> str:
        ...
        if any(
            critical in output_id.lower()
            for critical in ['navbar', 'main', 'error', 'auth', 'login']
        ):
            return 'HIGH'
    ...
    def _generate_summary(self) -> dict:
        ...
        return {
            ...
            'average_complexity': (
                sum(p.complexity_score for p in self.patterns)
                / max(total_callbacks, 1)
            ),
