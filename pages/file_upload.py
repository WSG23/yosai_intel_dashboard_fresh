...
UnifiedUploadComponent = _safe_import_upload_component()
...
def register_callbacks(manager: Any, controller=None) -> None:
    ...
    if not any(hasattr(manager, attr) for attr in ("register_callback", "unified_callback", "callback")):
    ...
    def _do_registration() -> None:
        if not hasattr(manager, "register_callback"):
            if hasattr(manager, "unified_callback"):
                manager.register_callback = manager.unified_callback
                ...
                if TrulyUnifiedCallbacks:
                    ...
                    logger.warning("Using standard Dash callbacks - advanced features unavailable")
            else:
                raise ValueError(f"Unsupported callback manager: {type(manager)}")

        # Type assertion for Pylance - we already checked for None above
        assert _upload_component is not None
        _upload_component.register_callbacks(manager, controller)

    _callback_registry.register_deduplicated(
        [f"{callback_prefix}_handle", f"{callback_prefix}_progress", f"{callback_prefix}_finalize"],
        _do_registration,
...
def check_upload_system_health() -> dict:
    ...
    return {
        "status": status,
        "components": [("Upload component loaded" if _import_error is None else "Upload component missing")],
