...
from analytics.analytics_controller import AnalyticsController, AnalyticsConfig, ConsolidatedCallbackManager, CallbackEvent
...
def create_test_data(n_records: int = 1000) -> pd.DataFrame:
    ...
    for i in range(n_records):
        ...
        # Inject anomalies (20% of data)
            ...
            door = np.random.choice(doors)
            result = 'Denied' if np.random.random() > 0.3 else 'Granted'  # Higher failure rate
...
def test_callback(callback_data):
    ...
    if callback_data.event_type == CallbackEvent.ANALYSIS_START:
        print(f"   🚀 Starting analysis with {callback_data.data.get('total_records', 0)} records")
    elif callback_data.event_type == CallbackEvent.ANALYSIS_COMPLETE:
        print(f"   ✅ Analysis completed in {callback_data.data.get('processing_time', 0):.2f}s")
        print(f"   📊 Modules: {', '.join(callback_data.data.get('modules_completed', []))}")
    elif callback_data.event_type == CallbackEvent.SECURITY_THREAT_DETECTED:
        print(f"   🚨 SECURITY THREAT: {callback_data.data.get('risk_level', 'unknown')} risk level detected!")
    elif callback_data.event_type == CallbackEvent.ANOMALY_DETECTED:
        print(f"   ⚠️  ANOMALIES: {callback_data.data.get('total_anomalies', 0)} anomalies detected")
    elif callback_data.event_type == CallbackEvent.BEHAVIOR_RISK_IDENTIFIED:
        print(f"   👤 BEHAVIOR RISK: {callback_data.data.get('high_risk_count', 0)} high-risk users identified")
    elif callback_data.event_type == CallbackEvent.TREND_CHANGE_DETECTED:
        print(f"   📈 TREND CHANGE: {callback_data.data.get('trend', 'unknown')} trend detected")
...
def verify_results(results):
    ...
    if results.security_patterns:
        ...
        print(f"   🔒 Security Score: {security_score:.1f}/100 (Risk: {risk_level}, Threats: {threat_count})")
    ...
    if results.access_trends:
        ...
        print(f"   📈 Access Trend: {trend} (Strength: {strength:.2f}, Change: {change_rate:.1f}%)")
    ...
    if results.user_behavior:
        ...
        print(f"   👥 User Analysis: {total_users} users, {high_risk} high-risk (Score: {behavior_score:.1f})")
    ...
    success_checks.append(("Analysis completed", results.status in ['success', 'partial_success']))
