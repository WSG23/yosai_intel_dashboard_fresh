...
class FloorEstimationService:
    ...
    def estimate_floors(self, data: List[Dict], session_id: str) -> Dict[str, Any]:
        ...
        try:
            if not data:
                result = {"success": True, "total_floors": 1, "confidence": 0.0, "method": "default"}
            ...
            floor_detections = {
                ...
                'numeric_analysis': self._detect_floors_by_numeric_analysis(all_text_fields),
            ...
            fallback_result = {"success": True, "total_floors": 1, "confidence": 0.0, "method": "error_fallback"}
    ...
    def _detect_floors_by_patterns(self, text_fields: List[str]) -> Dict[str, Any]:
        ...
        confidence = min(0.9, pattern_matches / len(text_fields)) if text_fields else 0.0
        return {
            'floors': max_floor,
    ...
    def _detect_floors_by_numeric_analysis(self, text_fields: List[str]) -> Dict[str, Any]:
        ...
        confidence = min(0.7, unique_floors / max(max_floor, 1) * 0.5) if max_floor > 1 else 0.0
        return {
            'floors': max_floor,
            'confidence': confidence,
    ...
    def _detect_floors_from_door_ids(self, data: List[Dict]) -> Dict[str, Any]:
        ...
        door_ids: List[str] = []
        possible_id_columns = ['door_id', 'device_id', 'door', 'device', 'location', 'area', 'reader']
        for record in data:
            ...
            for pattern in [
                ...
                for match in matches:
                    try:
                        ...
                        continue
        max_floor = max(floor_numbers) if floor_numbers else 1
        confidence = min(0.8, len(floor_numbers) / len(set(door_ids))) if door_ids else 0.0
        return {
            'floors': max_floor,
            'confidence': confidence,
            'found_floors': sorted(list(floor_numbers)),
