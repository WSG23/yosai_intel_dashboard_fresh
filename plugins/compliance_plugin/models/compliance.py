...
class ConsentLog(Base):
    ...
    granted_timestamp = Column(DateTime(timezone=True), nullable=False, default=func.now())
    ...
    legal_basis = Column(String(50), nullable=False, default='consent')  # GDPR Article 6
    created_at = Column(DateTime(timezone=True), nullable=False, default=func.now())
    
    # Ensure uniqueness per user/consent type/jurisdiction
    __table_args__ = (
        UniqueConstraint('user_id', 'consent_type', 'jurisdiction', 'is_active', 
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            'withdrawn_timestamp': self.withdrawn_timestamp.isoformat() if self.withdrawn_timestamp else None,
...
class DSARRequest(Base):
    ...
    due_date = Column(DateTime(timezone=True), nullable=False)  # 30 days GDPR, varies by jurisdiction
...
class ComplianceAuditLog(Base):
    ...
    actor_user_id = Column(String(50), nullable=False, index=True)  # Who performed the action
    target_user_id = Column(String(50), nullable=True, index=True)  # Whose data was affected
    action_type = Column(String(100), nullable=False, index=True)   # VIEW_PROFILE, DELETE_BIOMETRIC_DATA, etc.
    resource_type = Column(String(50), nullable=False)             # 'user_profile', 'biometric_data', etc.
...
class DataRetentionPolicy(Base):
    ...
    data_type = Column(String(100), unique=True, nullable=False)   # 'biometric_templates', 'access_logs', etc.
