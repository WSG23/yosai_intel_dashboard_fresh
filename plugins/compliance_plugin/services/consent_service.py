...
class ConsentServiceProtocol(Protocol):
    ...
    def grant_consent(self, user_id: str, consent_type: ConsentType, jurisdiction: str) -> bool:
        """Grant consent for specific processing"""
        ...
    
    def withdraw_consent(self, user_id: str, consent_type: ConsentType, jurisdiction: str) -> bool:
        """Withdraw previously granted consent"""
        ...
    
    def check_consent(self, user_id: str, consent_type: ConsentType, jurisdiction: str) -> bool:
...
class ConsentService:
    ...
    def grant_consent(
        ...
        try:
            ...
            if existing:
                logger.info(f"Consent already exists for {user_id}, {consent_type.value}, {jurisdiction}")
                return True
            ...
            logger.info(f"Consent granted: {user_id}, {consent_type.value}, {jurisdiction}")
            return True
            
        except IntegrityError as e:
            logger.error(f"Consent already exists or integrity constraint violated: {e}")
            return False
    ...
    def withdraw_consent(
        ...
        try:
            ...
            if not consent:
                logger.warning(f"No active consent found to withdraw: {user_id}, {consent_type.value}")
                return False
            ...
            rows_affected = self.db.execute_command(update_sql, (withdrawn_at, consent['id']))
            ...
            if rows_affected > 0:
                ...
                logger.info(f"Consent withdrawn: {user_id}, {consent_type.value}, {jurisdiction}")
                return True
    ...
    def _get_active_consent(
        ...
        try:
            ...
            df = self.db.execute_query(query_sql, (user_id, consent_type.value, jurisdiction))
    ...
    def _halt_processing_for_consent_type(self, user_id: str, consent_type: ConsentType):
    ...
    def _schedule_biometric_deletion(self, user_id: str):
        ...
        try:
            ...
            logger.info(f"Scheduled biometric deletion for {user_id} on {deletion_date}")
...
def create_consent_service(db: DatabaseProtocol, audit_logger: ComplianceAuditLogger) -> ConsentService:
