...
class BreachNotificationService:
    ...
    def __init__(self, db: DatabaseProtocol, audit_logger: ComplianceAuditLogger):
        ...
        self._high_risk_indicators = {
            'data_types': [
                'biometric_templates', 'health_data', 'genetic_data',
                'criminal_conviction_data', 'financial_data', 'children_data'
            ],
    ...
    def report_breach(
        ...
        """
        try:
            breach_id = f"BREACH-{datetime.now().strftime('%Y%m%d')}-{str(uuid4())[:8].upper()}"
            ...
            notification_requirements = self._determine_notification_requirements(
                severity, affected_data_types, estimated_affected_individuals,
                initial_assessment
            )
            ...
            breach_record = {
                ...
                'supervisor_notification_required': notification_requirements.get('supervisor_required', False),
                'individual_notification_required': notification_requirements.get('individuals_required', False),
            ...
            self.audit_logger.log_action(
                ...
                metadata={
                    'breach_category': breach_category.value,
                    'severity': severity.value,
                    'supervisor_notification_required': notification_requirements.get('supervisor_required'),
                    'notification_deadline_hours': deadlines.get('supervisor_notification_deadline_hours')
                }
            )
            ...
            logger.info(f"Data breach reported: {breach_id}, severity: {severity.value}")
            return breach_id
    ...
    def assess_notification_requirements(self, breach_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            assessment = breach_record.get('updated_assessment', breach_record.get('initial_assessment', {}))
    ...
    def notify_supervisory_authority(
        ...
        try:
            breach_record = self._get_breach_record(breach_id)
            if not breach_record:
                logger.error(f"Breach {breach_id} not found for supervisor notification")
                return False
            ...
            hours_since_detection = (datetime.now(timezone.utc) - detection_time).total_seconds() / 3600
            within_deadline = hours_since_detection <= self._notification_deadlines['supervisor_hours']
            ...
            notification_record = {
                ...
                'recipient': notification_details.get('authority_name', 'Data Protection Authority'),
                'notification_timestamp': datetime.now(timezone.utc),
                'notification_method': notification_details.get('method', 'online_portal'),
            ...
            self._update_breach_status(breach_id, NotificationStatus.SUPERVISOR_NOTIFIED)
    ...
    def notify_affected_individuals(
        ...
        try:
            ...
            # Update breach status
            self._update_breach_status(breach_id, NotificationStatus.INDIVIDUALS_NOTIFIED)
            ...
            logger.info(f"Notified {len(affected_user_ids)} individuals for breach {breach_id}")
            return True
    ...
    def get_breach_dashboard_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            high_severity_count = len([b for b in recent_breaches if b['severity'] in ['high', 'critical']])
    ...
    def _determine_notification_requirements(
        ...
        supervisor_required = (
            severity in [BreachSeverity.MEDIUM, BreachSeverity.HIGH, BreachSeverity.CRITICAL] or
            estimated_affected >= 100 or
            any(dt in self._high_risk_indicators['data_types'] for dt in affected_data_types)
        )
        ...
        individuals_required = (
            severity in [BreachSeverity.HIGH, BreachSeverity.CRITICAL] or
            assessment.get('identity_theft_risk', False) or
            assessment.get('financial_loss_risk', False) or
            assessment.get('discrimination_risk', False) or
            (estimated_affected >= 1000 and any(dt in self._high_risk_indicators['data_types'] for dt in affected_data_types))
        )
    ...
    def _calculate_notification_deadlines(self, detection_timestamp: datetime) -> Dict[str, Any]:
        ...
        return {
            ...
            'supervisor_notification_deadline_hours': self._notification_deadlines['supervisor_hours'],
    ...
    def _conduct_detailed_risk_assessment(
        ...
        rights_impact = {
            'privacy_impact': 'High' if any(dt in ['biometric_templates', 'health_data'] for dt in affected_data_types) else 'Medium',
            'autonomy_impact': 'Medium' if assessment.get('automated_decision_making', False) else 'Low',
            'discrimination_risk': 'High' if assessment.get('discrimination_risk', False) else 'Low',
            'identity_theft_risk': 'High' if assessment.get('identity_theft_risk', False) else 'Medium',
            'financial_loss_risk': 'High' if assessment.get('financial_loss_risk', False) else 'Low'
        }
        ...
        likelihood_factors = {
            ...
            'containment_effectiveness': assessment.get('containment_effectiveness', 'effective')
        }
        
        # Overall risk level
        high_impact_count = sum(1 for impact in rights_impact.values() if impact == 'High')
        overall_risk = 'High' if high_impact_count >= 2 else 'Medium' if high_impact_count >= 1 else 'Low'
        ...
        return {
            ...
            'data_sensitivity_assessment': {
                dt: 'High' if dt in self._high_risk_indicators['data_types'] else 'Medium'
...
def create_breach_notification_service(db: DatabaseProtocol, audit_logger: ComplianceAuditLogger) -> BreachNotificationService:
