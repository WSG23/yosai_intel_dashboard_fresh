...
class CrossBorderTransferServiceProtocol(Protocol):
    ...
    def assess_transfer_legality(self, destination_country: str, data_types: List[str]) -> Dict[str, Any]:
...
class CrossBorderTransferService:
    ...
    def __init__(self, db: DatabaseProtocol, audit_logger: ComplianceAuditLogger):
        ...
        self._adequacy_countries = {
            'AD', 'AR', 'CA', 'FO', 'GG', 'IL', 'IM', 'JP', 'JE', 'NZ', 'CH', 'UY', 'GB', 'US'
        }
    ...
    def assess_transfer_legality(
        ...
        try:
            ...
            required_mechanisms = self._determine_transfer_mechanisms(
                destination_country, sensitivity_assessment, is_high_risk, 
                transfer_relationship, data_subject_count
            )
            ...
            assessment = {
                ...
                'risk_level': self._calculate_transfer_risk_level(
                    has_adequacy, sensitivity_assessment, is_high_risk, data_subject_count
                ),
            ...
            logger.info(f"Transfer assessment completed: {assessment_id} to {destination_country}")
            return assessment
    ...
    def register_transfer_activity(
        ...
        try:
            ...
            if not self._validate_transfer_mechanism(assessment, transfer_mechanism):
                raise ValueError(f"Transfer mechanism {transfer_mechanism.value} not appropriate for this transfer")
            ...
            transfer_record = {
                ...
                'compliance_monitoring': {
                    'next_review_date': self._calculate_next_review_date(transfer_mechanism),
                    'monitoring_requirements': self._get_monitoring_requirements(transfer_mechanism),
                    'documentation_requirements': self._get_documentation_requirements(transfer_mechanism)
                },
    ...
    def get_transfer_dashboard_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            adequacy_transfers = sum(
                stat['transfer_count'] for stat in transfer_stats 
            ...
            return {
                'summary': {
                    ...
                    'adequacy_coverage_percentage': round((adequacy_transfers / max(total_transfers, 1)) * 100, 1)
                },
    ...
    def _assess_data_sensitivity(self, data_types: List[str]) -> Dict[str, Any]:
        ...
        max_score = max([sensitivity_scores.get(dt, 30) for dt in data_types], default=30)
        avg_score = sum([sensitivity_scores.get(dt, 30) for dt in data_types]) / len(data_types) if data_types else 30
        ...
        return {
            ...
            'data_type_scores': {dt: sensitivity_scores.get(dt, 30) for dt in data_types}
        }
    ...
    def _determine_transfer_mechanisms(
        ...
        if destination_country.upper() in self._adequacy_countries:
            ...
            if not is_high_risk and sensitivity_assessment['overall_level'] in ['low', 'medium']:
            ...
            elif sensitivity_assessment['overall_level'] in ['high', 'critical'] or is_high_risk:
    ...
    def _generate_compliance_requirements(
        ...
        for mechanism in mechanisms:
            if mechanism == TransferMechanism.STANDARD_CONTRACTUAL_CLAUSES:
                ...
                requirements['documentation_required'].extend([
                    'approved_bcr_documentation',
                    'bcr_compliance_monitoring_reports'
                ])
                requirements['monitoring_requirements'].extend([
                    'annual_bcr_compliance_review',
                    'data_subject_complaint_handling'
                ])
...
def create_cross_border_transfer_service(db: DatabaseProtocol, audit_logger: ComplianceAuditLogger) -> CrossBorderTransferService:
