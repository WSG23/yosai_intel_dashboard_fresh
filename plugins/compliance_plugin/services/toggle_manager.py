...
class BypassReason(Enum):
    ...
    LEGAL_BASIS_CONTRACT = "legal_basis_contract"   # Employment contract covers processing
...
class ComplianceToggleManager:
    ...
    def should_apply_compliance(
            ...
            if self._toggle_matches_context(toggle, context):
                ...
                if not toggle.enabled:
                    ...
                    result['bypassed_scopes'].append({
                        ...
                        'bypass_reason': toggle.bypass_reason.value if toggle.bypass_reason else 'not_specified'
                    })
        ...
        if result['bypassed_scopes']:
            ...
            global_bypass = any(
                scope['scope'] == 'global' or 
                (scope['scope'] == 'client' and scope['scope_value'] == context.get('client_id')) or
                (scope['scope'] == 'data_source' and scope['scope_value'] == context.get('data_source'))
    ...
    def _toggle_matches_context(self, toggle: ComplianceToggle, context: Dict[str, Any]) -> bool:
    ...
    def _determine_partial_compliance(
        self,
        context: Dict[str, Any],
        bypassed_scopes: List[Dict[str, Any]]
    ) -> bool:
        ...
        biometric_bypassed = any(
            scope['scope'] == 'data_type' and scope['scope_value'] in ['biometric_data', 'facial_recognition']
        ...
        consent_required_purposes = ['behavioral_analysis', 'location_tracking', 'marketing_analytics']
    ...
    def _add_bypass_warnings(self, result: Dict[str, Any], context: Dict[str, Any]) -> None:
        ...
        if any('biometric' in str(scope) for scope in result['bypassed_scopes']):
            ...
            if any('biometric' in dt for dt in data_types):
                warnings.append("WARNING: Biometric data processing bypassed compliance checks")
        
        # Warn about client bypasses without proper reason
        client_bypasses = [s for s in result['bypassed_scopes'] if s['scope'] == 'client']
        for bypass in client_bypasses:
            if bypass['bypass_reason'] == 'not_specified':
                warnings.append(f"WARNING: Client {bypass['scope_value']} bypass has no specified reason")
        ...
        try:
            ...
            for _, row in df.iterrows():
                toggle = ComplianceToggle(
                    ...
                    enabled=row['enabled'],
                    bypass_reason=BypassReason(row['bypass_reason']) if row['bypass_reason'] else None,
    ...
    def _store_toggle(self, toggle: ComplianceToggle) -> None:
        ...
        self.db.execute_command(upsert_sql, (
            toggle.id,
            toggle.scope.value,
            toggle.scope_value,
            toggle.enabled,
...
class CompliancePlugin(BasePlugin):
    ...
    def _hook_csv_pre_process(self, **kwargs) -> Dict[str, Any]:
        ...
        context = {
            ...
            'processing_purpose': kwargs.get('upload_context', {}).get('processing_purpose', ''),
...
def create_enhanced_csv_upload_with_toggles():
    ...
    def upload_csv_with_toggle_support():
        ...
        upload_context = {
            ...
            'compliance_bypass_reason': request.form.get('bypass_reason', ''),  # NEW: Manual bypass
            'dpo_approval': request.form.get('dpo_approval') == 'true'
        }
        
        try:
            # Call compliance hook (now toggle-aware)
            pre_result = call_plugin_hook('csv_upload_pre_process',
                file_path=file_path,
            ...
            if pre_result.get('compliance_bypassed'):
                logger.info(f"Compliance bypassed for upload by {current_user.id}: {pre_result.get('bypass_reasons')}")
            ...
            # Handle compliance denial
                ...
                return jsonify({
                    ...
                    'required_consents': pre_result.get('required_consents', []),
            ...
            # Normal compliance processing
                ...
                call_plugin_hook('csv_upload_post_process',
                    processing_id=processing_id,
...
def create_toggle_management_api():
    ...
    def create_compliance_toggle():
        ...
        try:
            ...
            bypass_reason = BypassReason(data['bypass_reason']) if data.get('bypass_reason') else None
