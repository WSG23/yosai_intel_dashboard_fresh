...
class ASTAnalyzer(BaseAnalyzer):
    ...
    def analyze(self, file_path: Path) -> List[QualityIssue]:
        ...
        try:
            ...
            issues.append(QualityIssue(
                file_path=str(file_path), line_number=1,
                issue_type=IssueType.STYLE, severity="error",
                message=f"Parse error: {e}", rule="parse_error"
            ))
        return issues
    ...
    def _analyze_function(self, node: ast.FunctionDef, file_path: Path) -> List[QualityIssue]:
        issues = []
        ...
        if complexity > self.max_complexity:
            issues.append(QualityIssue(
                ...
                issue_type=IssueType.COMPLEXITY, severity="warning",
        ...
        if func_length > self.max_function_length:
            issues.append(QualityIssue(
                file_path=str(file_path), line_number=node.lineno,
                issue_type=IssueType.MAINTAINABILITY, severity="warning",
    ...
    def _analyze_class(self, node: ast.ClassDef, file_path: Path) -> List[QualityIssue]:
        ...
        if not ast.get_docstring(node):
            issues.append(QualityIssue(
                file_path=str(file_path), line_number=node.lineno,
                issue_type=IssueType.DOCUMENTATION, severity="info",
