        ...
        def fallback_suggestions(columns):
            ...
            for col in columns:
                ...
                if any(
                    ...
                    keyword in col_lower for keyword in ["result", "status", "access"]
                ):
                    suggestion = {"field": "access_result", "confidence": 0.7}
                elif any(
                    keyword in col_lower for keyword in ["token", "badge", "card"]
                ):
        ...
        class FallbackDeviceMapping:
            def process_uploaded_data(self, df, client_profile="auto"):
                ...
                for col in df.columns:
                    if any(
                        device_col.lower() in col.lower() for device_col in device_cols
                    ):
        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        html.H3(
                            "📊 Step 2: Raw Data Preview (First 5 Rows)",
                            className="mb-3",
                        ),
            ...
            [
                dbc.CardBody(
                    [
                        ...
                        html.P(
                            f"AI Suggestion: {suggested_field} ({confidence:.1%} confidence)"
                            if suggested_field
        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        html.H3(
                            "🎯 Step 3: Column Mapping (AI + Manual)", className="mb-3"
                        ),
                        html.P(
                            "Map your data columns to standard fields. AI suggestions are pre-filled."
                        ),
            ...
            [
                dbc.CardBody(
                    [
                        ...
                        html.P(
                            f"AI Confidence: {ai_data.get('confidence', 0):.1%}"
                            if ai_data
                            else "No AI data"
                        ),
                        dbc.Row(
        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        html.H3(
                            "🏢 Step 4: Device Mapping (AI + Manual)", className="mb-3"
                        ),
                        html.P(
                            f"Configure properties for {len(devices)} devices. AI suggestions are pre-filled."
                        ),
                        ...
                        dbc.ButtonGroup(
                            [
                                ...
                                dbc.Button(
                                    "Apply AI Device Mapping",
                                    id="btn-apply-ai-devices",
                                    color="info",
                                ),
                                dbc.Button(
                                    "Next: Enhanced Data",
        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        html.H3(
                            "✨ Step 5: Enhanced Data Preview (First 5 Rows)",
                            className="mb-3",
                        ),
                        dbc.Alert(
                            [
                                html.H5(
                                    "Enhancement Summary:", className="alert-heading"
                                ),
...
@callbacks.callback(
    ...
    [State("upload-data", "filename"), State("session-store", "data")],
    callback_id="update_main_content",
    component_name="simple_mapping_interface",
)
    ...
    if trigger_id == "upload-data" and contents is not None:
        ...
        if error or df is None:
            return (
                ...
                0,
                stored_data,
                dbc.Alert(error or "Failed to load file", color="danger"),
            )

        stored_data.update(
            {"raw_df": df.to_dict("records"), "filename": filename, "stage": "raw_view"}
        )
        return (
            create_raw_data_stage(df),
            1,
            stored_data,
            dbc.Alert(f"Successfully loaded {filename}", color="success"),
        )
        ...
        if stored_data.get("raw_df"):
            ...
            enhanced_df, error = enhance_data_with_mappings(
                df, column_mapping, device_mapping
            )
            if error:
                return (
                    no_update,
...
@callbacks.callback(
    ...
    callback_id="update_column_mapping",
    component_name="simple_mapping_interface",
)
...
@callbacks.callback(
    ...
    prevent_initial_call=True,
    callback_id="update_device_mapping",
    component_name="simple_mapping_interface",
)
def update_device_mapping(
    floors, security_levels, access_types, special_props, ids, stored_data
):
    if not ids:
        return stored_data
    ...
    for i, device_id in enumerate([id_dict["index"] for id_dict in ids]):
        device_data = {
            "door_id": device_id,
            "floor_number": (
                floors[i] if i < len(floors) and floors[i] is not None else 1
            ),
            "security_level": (
                security_levels[i]
                if i < len(security_levels) and security_levels[i] is not None
                else 5
            ),
            "is_entry": "is_entry"
            in (access_types[i] if i < len(access_types) and access_types[i] else []),
            "is_exit": "is_exit"
            in (access_types[i] if i < len(access_types) and access_types[i] else []),
            "is_elevator": "is_elevator"
            in (
                special_props[i] if i < len(special_props) and special_props[i] else []
            ),
