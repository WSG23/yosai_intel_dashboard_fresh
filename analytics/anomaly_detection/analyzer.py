...
class SecurityCallbackController:
    """Consolidated callback controller for security events"""
    ...
    def emit_event(self, event_type: str, data: Dict[str, Any]) -> None:
        """Emit security event to all registered callbacks"""
        try:
...
class AnomalyDetector:
    ...
    def detect_anomalies(self, df: pd.DataFrame, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            self.callbacks.emit_event("DETECTION_ERROR", {"error": str(e)})
    ...
    def analyze_anomalies(self, df: pd.DataFrame, **kwargs) -> AnomalyResult:
        ...
        try:
            ...
            self.callbacks.emit_event("ANALYSIS_ERROR", {"error": str(e)})
            return self._empty_result()
    ...
    def _analyze_chunked(self, df: pd.DataFrame, **kwargs) -> AnomalyResult:
        ...
        self.callbacks.emit_event("CHUNKED_PROCESSING_START", {
            "total_rows": len(df),
        ...
        try:
            with ThreadPoolExecutor(max_workers=self.config.max_workers) as executor:
                ...
                for i in range(0, len(df), chunk_size):
                    ...
                    future = executor.submit(self._analyze_chunk, chunk, i // chunk_size)
            ...
            return self._consolidate_results(all_anomalies, {
            ...
            self.callbacks.emit_event("CHUNKED_PROCESSING_ERROR", {"error": str(e)})
            return self._empty_result()
    ...
    def _analyze_full_dataset(self, df: pd.DataFrame, **kwargs) -> AnomalyResult:
        ...
        try:
            ...
            return self._consolidate_results(anomalies, {
    ...
    def _analyze_chunk(self, chunk: pd.DataFrame, chunk_id: int) -> List[Dict[str, Any]]:
    ...
    def _consolidate_results(self, anomalies: List[Dict[str, Any]], metadata: Dict[str, Any]) -> AnomalyResult:
        ...
        try:
            ...
            # Emit events
            self.callbacks.emit_event("ANOMALIES_DETECTED", {
    ...
    def _generate_detection_summary(self, anomalies: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "confidence_mean": np.mean([a.get('confidence', 0) for a in anomalies]) if anomalies else 0
        }
    ...
    def _generate_recommendations(self, anomalies: List[Dict[str, Any]], risk_assessment: Dict[str, Any]) -> List[str]:
        ...
        if severity_dist["critical"] > 0:
            recommendations.append("IMMEDIATE: Investigate critical anomalies requiring urgent attention")
        
        if severity_dist["high"] > 0:
            recommendations.append("HIGH PRIORITY: Review high-severity anomalies within 24 hours")
        
        if len(anomalies) > 50:
            recommendations.append("Consider implementing automated response for high-volume anomalies")
        
        # Method-specific recommendations
        methods = set(a.get('method', '') for a in anomalies)
        if 'frequency' in methods:
            recommendations.append("Monitor user activity patterns for potential insider threats")
    ...
    def _empty_legacy_result(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "severity_distribution": {"critical": 0, "high": 0, "medium": 0, "low": 0},
            "detection_summary": {"total_anomalies": 0, "message": "Error in detection"},
