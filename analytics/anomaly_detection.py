...
class AnomalyDetector:
        ...
        try:
            ...
            for user_id, access_count in outlier_users.items():
                ...
                anomaly_type = (
                    ...
                    if access_count > mean_access
                    else "insufficient_access_frequency"
                )
    ...
    def _detect_iqr_anomalies(
        ...
        try:
            ...
            outlier_days = daily_access[
                (daily_access < lower_bound) | (daily_access > upper_bound)
            ]
    ...
    def _detect_rapid_access_patterns(
        self, df: pd.DataFrame, sensitivity: float
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            df_sorted["time_diff"] = (
                df_sorted.groupby("person_id")["timestamp"].diff().dt.total_seconds()
            )
                ...
                for user_id, count in user_rapid_counts.items():
                    if count >= 2:
                        ...
                        severity = (
                            "critical"
                            if failure_rate > 0.8
                            else "high" if failure_rate > 0.5 else "medium"
                        )
                        ...
                        anomalies.append(
                            {
                                ...
                                "evidence": {
                                    ...
                                    "attempts_details": user_rapid_data[
                                        ["timestamp", "door_id", "access_result"]
                                    ].to_dict("records"),
                                },
                                "timestamp": datetime.now(),
                            }
    ...
    def _detect_badge_anomalies(
        ...
        try:
            ...
            user_badge_stats = df.groupby("person_id")["badge_status"].agg(
                ["count", lambda x: (x != "Valid").mean()]
            )
    ...
    def _isolation_forest_detection(
        self, features_df: pd.DataFrame, sensitivity: float, original_df: pd.DataFrame
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_detection_summary(
        ...
        return {
            ...
            "detection_methods": list(
                set([a.get("type", "unknown") for a in anomalies])
            ),
        }
    ...
    def _assess_overall_risk(self, anomalies: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        normalized_risk_score = (
            min(100, (risk_score / max_possible_score) * 100)
    ...
    def _generate_recommendations(
        ...
        if not anomalies:
            recommendations.append(
                "No anomalies detected. System appears to be functioning normally."
            )
        ...
        if len(recommendations) == 0:
            recommendations.append(
                "Monitor detected anomalies and investigate as needed."
            )
    ...
    def _convert_to_legacy_format(self, result: AnomalyAnalysis) -> Dict[str, Any]:
        ...
        return {
            ...
            "statistical_anomalies": [
                a
                for a in result.detection_summary.get("type_breakdown", {}).keys()
                if "zscore" in a or "iqr" in a or "mad" in a
            ],
            "pattern_anomalies": [
                a
                for a in result.detection_summary.get("type_breakdown", {}).keys()
                if "rapid" in a or "badge" in a
            ],
            "ml_anomalies": [
                a
                for a in result.detection_summary.get("type_breakdown", {}).keys()
            ...
            "confidence_mean": result.detection_summary.get("confidence_stats", {}).get(
                "mean", 0
            ),
        }
