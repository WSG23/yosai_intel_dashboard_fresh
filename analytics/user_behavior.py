...
class UserBehaviorAnalyzer:
    ...
    def analyze_user_behavior(self, df: pd.DataFrame) -> BehaviorAnalysis:
        ...
        try:
            ...
            insights = self._generate_behavioral_insights(
                user_features, global_patterns, high_risk_count
            )
    ...
    def _extract_temporal_features(self, user_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Activity span
        activity_span = (
            user_data["timestamp"].max() - user_data["timestamp"].min()
        ).days
    ...
    def _extract_regularity_features(self, user_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if len(time_gaps) > 0:
            gap_cv = time_gaps.std() / (
                time_gaps.mean() + 1e-10
    ...
    def _extract_risk_features(
        self, user_data: pd.DataFrame, full_df: pd.DataFrame
    ) -> Dict[str, Any]:
    ...
    def _calculate_anomalous_timing(
        self, user_data: pd.DataFrame, full_df: pd.DataFrame
    ) -> float:
        ...
        try:
            ...
            user_hour_dist = (
                ...
                .reindex(range(24), fill_value=1e-10)
            )
            global_hour_dist = (
                full_df["hour"]
                .value_counts(normalize=True)
    ...
    def _analyze_global_patterns(
        ...
        try:
            ...
            global_patterns["access_patterns"] = {
                "most_used_door": (
                    df["door_id"].mode().iloc[0] if len(df) > 0 else "Unknown"
                ),
                "door_usage_distribution": df["door_id"]
                .value_counts()
                .head(5)
                .to_dict(),
            ...
            global_patterns["risk_patterns"] = {
                ...
                "high_risk_user_percentage": (risk_scores > self.risk_threshold).mean()
                * 100,
    ...
    def _generate_behavioral_insights(
        ...
        try:
            ...
            if avg_events > 100:
                insights.append(
                    "Users show high overall activity levels, indicating heavy system usage."
                )
            elif avg_events < 10:
                insights.append(
                    "Users show low overall activity levels, indicating light system usage."
                )
            ...
            if avg_regularity > 0.8:
                insights.append(
                    "Users demonstrate highly regular access patterns, indicating predictable behavior."
                )
            elif avg_regularity < 0.3:
                insights.append(
                    "Users show irregular access patterns, suggesting diverse work schedules or roles."
                )
            ...
            if avg_doors > 5:
                insights.append(
                    "Users access multiple doors on average, indicating diverse access requirements."
                )
    ...
    def _generate_behavioral_recommendations(
        self, high_risk_count: int, global_patterns: Dict[str, Any]
    ) -> List[str]:
    ...
    def _convert_to_legacy_format(self, result: BehaviorAnalysis) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_accesses_per_user": result.global_patterns.get(
                "activity_distribution", {}
            ).get("avg_events_per_user", 0),
            "heavy_users": result.high_risk_users,  # Approximation
            "behavior_score": 100
            - (result.high_risk_users / max(result.total_users_analyzed, 1) * 100),
    ...
    def _empty_legacy_result(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "global_patterns": {},
            "insights": [
                "Unable to perform behavior analysis due to insufficient data"
            ],
            "recommendations": [
                "Collect more user access data for meaningful analysis"
            ],
            "user_segments": {
                "power_users": 0,
                "regular_users": 0,
