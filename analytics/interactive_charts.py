...
class SecurityChartsGenerator:
        ...
        for layer in layers:
            if layer["doors"]:
                ...
                metrics = {
                    ...
                    "success_rate": (
                        (layer_data["access_result"] == "Granted").mean() * 100
                        if len(layer_data) > 0
                        else 0
                    ),
                    "unique_users": (
                        layer_data["person_id"].nunique() if len(layer_data) > 0 else 0
                    ),
                    "failed_attempts": len(
                        layer_data[layer_data["access_result"] == "Denied"]
                    ),
                    "doors": layer["doors"],
                    "color": layer["color"],
                    "radius": layer["radius"],
    ...
    def _create_onion_visualization(self, layer_metrics: List[Dict]) -> go.Figure:
        ...
        fig.update_layout(
            ...
            xaxis=dict(
                range=[-120, 120], showgrid=False, showticklabels=False, zeroline=False
            ),
            yaxis=dict(
                range=[-120, 120],
                showgrid=False,
    ...
    def _create_layer_breakdown_chart(self, layer_metrics: List[Dict]) -> go.Figure:
        ...
        fig.update_layout(
            title="Security Layer Breakdown Analysis", showlegend=False, height=600
        )

        return fig

    def _create_security_overview_charts(
        ...
        charts["access_results"] = go.Figure(
            data=[
                go.Pie(
        ...
        charts["success_trend"].add_trace(
            go.Scatter(
        ...
        charts["activity_heatmap"] = go.Figure(
            data=go.Heatmap(
    ...
    def _create_temporal_charts(self, df: pd.DataFrame) -> Dict[str, go.Figure]:
        ...
        charts["time_series"].update_layout(
            title="Temporal Analysis Dashboard", height=600, showlegend=False
        )
        ...
        weekend_comparison = (
            df.groupby(["is_weekend", "hour"])["event_id"]
            .count()
    ...
    def _create_user_activity_charts(self, df: pd.DataFrame) -> Dict[str, go.Figure]:
        ...
        charts["user_distribution"].add_trace(
            go.Histogram(
        ...
        sizes = np.clip(
            user_activity["Total Events"] / user_activity["Total Events"].max() * 50,
            5,
            50,
        )

        charts["user_behavior"].add_trace(
            go.Scatter(
    ...
    def _create_door_analysis_charts(self, df: pd.DataFrame) -> Dict[str, go.Figure]:
        ...
        charts["door_security"].add_trace(
            go.Scatter(
        ...
        if len(failed_by_door) > 0:
            ...
            charts["door_failures"].add_trace(
                go.Bar(
    ...
    def _create_risk_dashboard(self, df: pd.DataFrame) -> Dict[str, go.Figure]:
        ...
        after_hours_rate = (
            (after_hours_events / total_events) * 100 if total_events > 0 else 0
        )
        weekend_rate = (weekend_events / total_events) * 100 if total_events > 0 else 0

        # Overall risk score calculation
        risk_score = min(
            100, failure_rate * 2 + after_hours_rate * 0.5 + weekend_rate * 0.3
        )

        charts["risk_gauge"] = go.Figure(
            go.Indicator(
        ...
        for i, (factor, value) in enumerate(risk_factors.items()):
            charts["risk_breakdown"].add_trace(
                go.Bar(x=[factor], y=[value], marker_color=colors[i], name=factor)
            )
    ...
    def _create_anomaly_charts(self, df: pd.DataFrame) -> Dict[str, go.Figure]:
        ...
        if len(daily_volumes) > 3:
            ...
            charts["volume_anomalies"].add_trace(
                go.Scatter(
                    x=normal_days.index,
        ...
        if len(user_activity) > 3:
            ...
            if len(anomalous_users) > 0:
                ...
                charts["user_anomalies"].add_trace(
                    go.Bar(
    ...
    def _create_compliance_charts(self, df: pd.DataFrame) -> Dict[str, go.Figure]:
        ...
        business_hours_compliance = df.groupby("date").agg(
            {"is_business_hours": "mean", "event_id": "count"}
        )
        business_hours_compliance["compliance_score"] = (
            business_hours_compliance["is_business_hours"] * 100
        )

        charts["time_compliance"] = go.Figure()
        charts["time_compliance"].add_trace(
            go.Scatter(
        ...
        if "badge_status" in df.columns:
            ...
            charts["badge_compliance"].add_trace(
                go.Scatter(
        ...
        compliance_metrics = {
            ...
            "Valid Badge Usage": (
                (df["badge_status"] == "Valid").mean() * 100
                if "badge_status" in df.columns
                else 100
            ),
        }
        ...
        charts["compliance_summary"].add_trace(
            go.Bar(
    ...
    def _calculate_onion_security_score(
        self, layer_metrics: List[Dict]
    ) -> Dict[str, Any]:
