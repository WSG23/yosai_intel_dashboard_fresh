...
def show_metadata_enhancement():
    ...
    st.markdown(
        "### Enrich your access control system with detailed user profiles and device metadata"
    )
...
def show_user_profiles():
    ...
    try:
        ...
        if parquet_path.exists():
            ...
            st.info(
                f"Found {len(unique_employees)} employees and {len(unique_cards)} access cards in your data"
            )
            ...
            with col1:
                ...
                # Add new user button
                if st.button(
                    "➕ Create New User Profile", help="Create New User Profile"
                ):
            ...
            with col2:
                if selected_employee:
                    show_user_profile_form(
                        selected_employee, user_profiles, unique_cards
                    )
            ...
            if user_profiles:
                ...
                st.info(
                    "No user profiles created yet. Create profiles to enhance your access control metadata."
                )

        else:
            st.warning(
                "Enhanced Security Demo data not found. Please ensure the data is loaded."
            )
    ...
    with st.form(f"user_profile_{employee_code}"):
        ...
        with col1:
            full_name = st.text_input(
                "Full Name", value=existing_profile.get("full_name", "")
            )
            department = st.selectbox(
                ...
                index=[
                    ...
                    "Legal",
                    "Other",
                ].index(existing_profile.get("department", "Other")),
            )
            job_title = st.text_input(
                "Job Title", value=existing_profile.get("job_title", "")
            )
        ...
        with col2:
            ...
            employee_type = st.selectbox(
                ...
                index=[
                    ...
                    "Temporary",
                    "Intern",
                ].index(existing_profile.get("employee_type", "Full-time")),
            )
            ...
            assigned_card = st.selectbox(
                ...
                index=(
                    0
                    if existing_profile.get("assigned_card") not in available_cards
                    else list(available_cards).index(
                        existing_profile.get("assigned_card")
                    )
                    + 1
                ),
            )
            ...
            manager = st.text_input(
                "Manager", value=existing_profile.get("manager", "")
            )
        ...
        with col4:
            office_location = st.text_input(
                "Office Location", value=existing_profile.get("office_location", "")
            )
...
def show_device_enhancement():
    ...
    try:
        ...
        enhanced_mappings = [
            ...
            if "Enhanced_Security_Demo" in str(v.get("filename", ""))
        ]
        ...
        if enhanced_mappings:
            ...
            with col1:
                ...
                if st.button("🔄 Refresh Device List", help="Refresh Device List"):
                    st.experimental_rerun()

            with col2:
                if selected_device:
                    show_device_enhancement_form(
                        selected_device,
                        device_mappings[selected_device],
                        enhanced_devices,
                    )
    ...
    with st.form(f"device_enhancement_{device_name}"):
        ...
        with col1:
            ...
            building = st.text_input(
                "Building", value=existing_enhanced.get("building", "Main Building")
            )
            floor_name = st.text_input(
                "Floor Name",
                value=existing_enhanced.get(
                    "floor_name",
                    f"Floor {original_mapping.get('floor_number', 'Unknown')}",
                ),
            )
            zone = st.text_input("Zone/Area", value=existing_enhanced.get("zone", ""))
            room_number = st.text_input(
                "Room Number", value=existing_enhanced.get("room_number", "")
            )
            ...
            device_type = st.selectbox(
                ...
                index=[
                    ...
                    "Gate",
                    "Other",
                ].index(existing_enhanced.get("device_type", "Card Reader")),
            )

            manufacturer = st.text_input(
                "Manufacturer", value=existing_enhanced.get("manufacturer", "")
            )
            model = st.text_input("Model", value=existing_enhanced.get("model", ""))
            serial_number = st.text_input(
                "Serial Number", value=existing_enhanced.get("serial_number", "")
            )
        ...
        with col2:
            ...
            enhanced_security_level = st.slider(
                ...
                max_value=10,
                value=existing_enhanced.get(
                    "enhanced_security_level", original_mapping.get("security_level", 5)
                ),
            )
            ...
            access_hours = st.selectbox(
                ...
                index=[
                    ...
                    "Restricted Hours",
                    "Emergency Only",
                ].index(existing_enhanced.get("access_hours", "24/7")),
            )

            requires_escort = st.checkbox(
                "Requires Escort", value=existing_enhanced.get("requires_escort", False)
            )
            dual_auth_required = st.checkbox(
                "Dual Authentication Required",
                value=existing_enhanced.get("dual_auth_required", False),
            )

            st.markdown("##### 📊 Monitoring")
            camera_coverage = st.checkbox(
                "Camera Coverage", value=existing_enhanced.get("camera_coverage", True)
            )
            logging_enabled = st.checkbox(
                "Access Logging", value=existing_enhanced.get("logging_enabled", True)
            )
            alert_enabled = st.checkbox(
                "Security Alerts", value=existing_enhanced.get("alert_enabled", False)
            )
        ...
        with col3:
            ...
            if existing_maintenance:
                try:
                    default_maintenance = datetime.fromisoformat(
            ...
            installation_date = st.date_input(
                "Installation Date", value=default_install
            )
            last_maintenance = st.date_input(
                "Last Maintenance", value=default_maintenance
            )

        with col4:
            responsible_person = st.text_input(
                "Responsible Person",
                value=existing_enhanced.get("responsible_person", ""),
            )
...
def show_user_device_relationships():
    ...
    if users and devices:
        ...
        with st.expander("🔧 Bulk Permission Settings", expanded=False):
            ...
            with col2:
                bulk_permission = st.selectbox(
                    "Permission Level:", ["Denied", "Granted", "Conditional"]
                )
            with col3:
            ...
            for i, device in enumerate(devices):
                with cols[i % 3]:
                    ...
                    new_permission = st.selectbox(
                        ...
                        index=["Not Set", "Denied", "Granted", "Conditional"].index(
                            current_permission
                        ),
        ...
        if permission_data:
            ...
            with col4:
                conditional_count = len(
                    perms_df[perms_df["Permission"] == "Conditional"]
                )
            ...
            with col1:
                ...
                dept_perms = (
                    perms_df.groupby(["Department", "Permission"])
                    .size()
            ...
            with col2:
                ...
                security_clearance = perms_df[
                    perms_df["Permission"] == "Granted"
                ].copy()
...
def show_metadata_analytics():
    ...
    if enhanced_devices:
        ...
        with col2:
            ...
            security_counts = (
                devices_df["enhanced_security_level"].value_counts().sort_index()
            )
            fig = px.bar(
    ...
    if user_profiles and enhanced_devices and permissions:
                        ...
                        compliance_issues.append(
                            {
                                ...
                                "Risk Level": (
                                    "High"
                                    if (device_security - user_clearance) > 2
                                    else "Medium"
                                ),
                            }
                        )
        ...
        if compliance_issues:
            st.warning(
                f"⚠️ Found {len(compliance_issues)} potential security compliance issues"
            )
