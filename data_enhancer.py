#!/usr/bin/env python3
"""
MVP Data Enhancement Tool - Standalone with Multi-Building Analysis
Location: core/mvp_data_enhance.py

Advanced standalone tool for multi-building data enhancement with annex support.
Based on latest GitHub updates and enhanced building detection patterns.
"""
...
class AdvancedUnicodeHandler:
    ...
    def clean_dataframe(df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Clean column names
        cleaned_df.columns = [AdvancedUnicodeHandler.clean_unicode_surrogates(str(col)) 
        ...
        for col in cleaned_df.columns:
            if cleaned_df[col].dtype == 'object':
                cleaned_df[col] = cleaned_df[col].astype(str).apply(
                    AdvancedUnicodeHandler.clean_unicode_surrogates
                )
...
class EnhancedFileProcessor:
    ...
    ENCODINGS = ['utf-8', 'latin1', 'cp1252', 'iso-8859-1', 'utf-16', 'shift_jis', 'euc-jp']
    
    @staticmethod
    def decode_contents(contents: str, filename: str) -> Tuple[Optional[pd.DataFrame], str]:
...
class MultiBuildingDataEnhancer:
    ...
    STANDARD_FIELDS = [
        'person_id', 'door_id', 'access_result', 'timestamp', 'token_id', 'event_type',
        'facility_id', 'building_id', 'zone_id', 'floor', 'direction'
    ]
    ...
    def get_enhanced_column_suggestions(df: pd.DataFrame) -> Dict[str, Dict]:
        ...
        if AI_COLUMN_SERVICE_AVAILABLE:
            try:
                from services.data_enhancer import get_ai_column_suggestions
                return get_ai_column_suggestions(df, MultiBuildingDataEnhancer.STANDARD_FIELDS)
            except Exception as e:
        ...
        field_keywords = {
            ...
            'door_id': ['door', 'entrance', 'exit', 'gate', 'portal', 'device', 'reader'],
            'access_result': ['result', 'status', 'outcome', 'success', 'granted', 'decision'],
    ...
    def get_enhanced_device_suggestions(df: pd.DataFrame) -> Dict[str, Dict]:
        ...
        for door in unique_doors:
            ...
            building_name = MultiBuildingDataEnhancer._generate_building_name(building_type, door_str)
            ...
            security_level = MultiBuildingDataEnhancer._calculate_security_level(door_str, building_type, floor_number)
            ...
            special_props = MultiBuildingDataEnhancer._detect_special_properties(door_str)
    ...
    def _detect_building_type(door_str: str) -> str:
        ...
        for building_type, patterns in MultiBuildingDataEnhancer.BUILDING_PATTERNS.items():
    ...
    @staticmethod
    def _calculate_security_level(door_str: str, building_type: str, floor_number: int) -> int:
        ...
        security_level = base_security + building_security_modifiers.get(building_type, 0)
        ...
        high_security_areas = ['server', 'data', 'executive', 'ceo', 'finance', 'secure', 'restricted']
    ...
    def _detect_access_types(door_str: str) -> Dict[str, bool]:
        ...
        return {
            'is_entry': any(x in door_str for x in ['entry', 'entrance', 'main', 'front', 'in']),
    ...
    def _detect_special_properties(door_str: str) -> Dict[str, bool]:
        ...
        return {
            ...
            'is_stairwell': any(x in door_str for x in ['stair', 'stairs', 'stairwell']),
            'is_fire_escape': any(x in door_str for x in ['fire', 'emergency', 'escape']),
            'is_restricted': any(x in door_str for x in ['restricted', 'secure', 'private', 'authorized'])
        }
    ...
    @staticmethod
    def apply_device_mappings(df: pd.DataFrame, device_mappings: Dict[str, Dict]) -> pd.DataFrame:
...
def create_enhanced_header():
    ...
    if AI_COLUMN_SERVICE_AVAILABLE:
        service_badges.append(dbc.Badge("AI Column ✅", color="success", className="me-2"))
    else:
        service_badges.append(dbc.Badge("AI Column ⚠️", color="warning", className="me-2"))
    
    if AI_DOOR_SERVICE_AVAILABLE:
        service_badges.append(dbc.Badge("AI Door ✅", color="success", className="me-2"))
    else:
    ...
    if CONTAINER_AVAILABLE:
        service_badges.append(dbc.Badge("Services ✅", color="success", className="me-2"))
    else:
        service_badges.append(dbc.Badge("Services ⚠️", color="warning", className="me-2"))
    
    # Add multi-building badge
    service_badges.append(dbc.Badge("Multi-Building 🏢", color="info", className="me-2"))
    
    return dbc.Card([
        dbc.CardBody([
            html.H1("MVP Data Enhancement Tool", 
                   className="text-center text-primary mb-2"),
            html.P("Advanced Multi-Building Analysis - Core Testing", 
                   className="text-center text-muted mb-3"),
            html.Div(service_badges, className="text-center mb-3"),
            html.P("Upload, map, enhance, and export multi-building access control data",
                   className="text-center text-muted mb-4"),
            dbc.Progress(id="progress-bar", value=20, striped=True, animated=True)
        ])
    ], style=CARD_STYLE)
...
def create_upload_section():
    ...
    return dbc.Card([
        dbc.CardHeader(html.H4("Step 1: File Upload & Multi-Building Detection", className="mb-0")),
        dbc.CardBody([
            dcc.Upload(
                id='upload-data',
                children=html.Div([
                    ...
                    html.P("Enhanced encoding support + building detection", className="text-muted small")
...
def create_preview_section():
    ...
    return dbc.Card([
        dbc.CardHeader(html.H4("Step 2: Data Preview & Building Analysis", className="mb-0")),
        dbc.CardBody([
...
def create_column_mapping_section():
    ...
    return dbc.Card([
        dbc.CardHeader(html.H4("Step 3: Enhanced Column Mapping", className="mb-0")),
        dbc.CardBody([
            dbc.Row([
                dbc.Col([
                    dbc.Button("Get AI Suggestions", id="ai-column-btn", 
                              color="primary", className="mb-3"),
                    html.Div(id="ai-column-status")
                ], width=6),
                dbc.Col([
                    html.P("Enhanced Standard Fields:", className="font-weight-bold"),
                    html.Ul([html.Li(field) for field in MultiBuildingDataEnhancer.STANDARD_FIELDS])
                ], width=6)
            ]),
            html.Hr(),
...
def create_device_mapping_section():
    ...
    return dbc.Card([
        dbc.CardHeader(html.H4("Step 4: Multi-Building Device Mapping", className="mb-0")),
        dbc.CardBody([
            dbc.Row([
                dbc.Col([
                    dbc.Button("Get AI Device Suggestions", id="ai-device-btn", 
                              color="success", className="mb-3"),
                    html.Div(id="ai-device-status")
                ], width=6),
                dbc.Col([
                    html.P("Building Types Detected:", className="font-weight-bold"),
...
def create_enhancement_section():
    ...
    return dbc.Card([
        dbc.CardHeader(html.H4("Step 5: Multi-Building Data Enhancement", className="mb-0")),
        dbc.CardBody([
            dbc.Button("Enhance Data", id="enhance-btn", 
...
def create_export_section():
    ...
    return dbc.Card([
        dbc.CardHeader(html.H4("Step 6: Export Enhanced Multi-Building Data", className="mb-0")),
        dbc.CardBody([
            dbc.Row([
                dbc.Col([
                    dbc.Button("Download CSV", id="download-csv-btn", 
                              color="info", className="mb-2"),
                    dcc.Download(id="download-csv")
                ], width=4),
                dbc.Col([
                    dbc.Button("Download JSON", id="download-json-btn", 
                              color="info", className="mb-2"),
                    dcc.Download(id="download-json")
                ], width=4),
                dbc.Col([
                    dbc.Button("Building Summary", id="download-summary-btn", 
...
def analyze_buildings_in_data(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for building_type, count in building_types.items():
        building_name = MultiBuildingDataEnhancer._generate_building_name(building_type, "")
...
def create_building_analysis_display(analysis: Dict[str, Any]) -> html.Div:
    ...
    for building_info in analysis['potential_buildings']:
        card = dbc.Card([
            dbc.CardBody([
                ...
                html.P(f"Type: {building_info['type']}", className="text-muted small"),
                html.P(f"Doors: {building_info['door_count']}", className="mb-0")
            ])
    ...
    for floor, count in analysis['floor_distribution'].items():
        floor_name = f"Floor {floor}" if floor > 0 else "Basement" if floor < 0 else "Ground"
        floor_items.append(html.Li(f"{floor_name}: {count} doors"))
    
    return html.Div([
        html.H6("🏢 Multi-Building Analysis Results:"),
        html.P(f"Total Unique Doors: {analysis['total_unique_doors']}", className="mb-3"),
        
        html.H6("Building Distribution:"),
        dbc.Row(building_cards, className="mb-3"),
        
        html.H6("Floor Distribution:"),
        html.Ul(floor_items) if floor_items else html.P("No floor data detected", className="text-muted")
    ])
...
def handle_enhanced_column_mapping(n_clicks):
    ...
    for field in MultiBuildingDataEnhancer.STANDARD_FIELDS:
        ...
        control = dbc.Row([
            dbc.Col([
                dbc.Label(label_text, className=label_class)
            ], width=4),
            dbc.Col([
                dcc.Dropdown(
                    id=f'mapping-{field}',
...
def handle_enhanced_device_mapping(n_clicks):
    ...
    if 'door_id' not in df.columns:
        return "", dbc.Alert("No 'door_id' column found for device mapping", color="warning"), ""
    ...
    for building_type, count in building_types.items():
        ...
        building_summary.append(
            dbc.Badge(f"{building_type.title()}: {count}", color=badge_color, className="me-2 mb-1")
        )
    ...
    for door_id in unique_doors:
        ...
        door_card = dbc.Card([
            dbc.CardHeader([
                ...
                html.Small(f"Building: {door_data.get('building', 'Unknown')}", className="text-muted")
            ]),
            dbc.CardBody([
                dbc.Row([
                    dbc.Col([
                ...
                dbc.Row([
                    dbc.Col([
                        ...
                        dbc.Checklist(
                            ...
                            value=[k for k, v in door_data.items() 
...
@app.callback(
    ...
    [State(f'mapping-{field}', 'value') for field in MultiBuildingDataEnhancer.STANDARD_FIELDS],
...
def handle_enhanced_data_enhancement(n_clicks, *mapping_values):
    ...
    try:
        ...
        enhanced_df = MultiBuildingDataEnhancer.apply_column_mappings(df, column_mappings)
        
        # Apply device mappings
        enhanced_df = MultiBuildingDataEnhancer.apply_device_mappings(enhanced_df, session_data.get('device_mappings', {}))
        ...
        building_count = len(enhanced_df['building'].unique()) if 'building' in enhanced_df.columns else 0
        floor_count = len(enhanced_df['floor_number'].unique()) if 'floor_number' in enhanced_df.columns else 0
        ...
        preview = [
            ...
            html.P("Building columns are highlighted in blue, other new columns in green", className="text-muted small"),
            dash_table.DataTable(
...
def handle_enhanced_downloads(csv_clicks, json_clicks, summary_clicks):
    ...
    if button_id == "download-csv-btn" and csv_clicks:
        ...
        return None, dcc.send_data_frame(
...
"""
    ...
    if 'floor_number' in df.columns:
        ...
        for floor, count in floor_stats.items():
            floor_name = f"Floor {floor}" if floor > 0 else "Basement" if floor < 0 else "Ground Floor"
            percentage = (count / len(df)) * 100
    ...
    special_props = ['is_entry', 'is_exit', 'is_elevator', 'is_stairwell', 'is_fire_escape', 'is_restricted']
...
if __name__ == '__main__':
    ...
    print(f"📍 Tool Location: {__file__}")
    print(f"🔧 AI Column Service: {'✅ Available' if AI_COLUMN_SERVICE_AVAILABLE else '⚠️ Enhanced Fallback'}")
    print(f"🚪 AI Door Service: {'✅ Available' if AI_DOOR_SERVICE_AVAILABLE else '⚠️ Enhanced Fallback'}")
    print(f"⚙️ Config Service: {'✅ Available' if CONFIG_SERVICE_AVAILABLE else '⚠️ Fallback'}")
    print(f"🔌 Service Container: {'✅ Available' if CONTAINER_AVAILABLE else '⚠️ Not Available'}")
    print(f"🏢 Multi-Building Support: ✅ Enhanced with Annex Detection")
    print(f"🏗️ Floor Detection: ✅ Advanced Patterns (F01-F99, Basements)")
    print(f"🔒 Security Levels: ✅ Context-Aware (1-10 scale)")
    print("=" * 70)
