...
async def process_file_with_service(file_path: str, verbose: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        df = await processor.process_file(content_with_prefix, path.name, progress_callback)
        ...
        logger.info(f"AsyncFileProcessor succeeded: {len(df)} rows, {len(df.columns)} columns")
        return result
