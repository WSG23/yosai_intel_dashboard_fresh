...
async def test_analytics_with_fix():
    try:
        ...
        try:
            access_patterns = analytics.analyze_access_patterns(days=1)  # Use 1 day since our data is from one day
            result["tests"]["access_patterns"] = {"success": True, "result": access_patterns}
            print(f"✅ Access patterns found: {len(access_patterns.get('patterns', []))}")
        except Exception as e:
        ...
        try:
            ...
            result["tests"]["anomaly_detection"] = {"success": True, "result": anomalies}
            print(f"✅ Anomalies detected: {len(anomalies) if isinstance(anomalies, list) else 'N/A'}")
        except Exception as e:
        ...
        try:
            ...
            result["tests"]["data_processing"] = {"success": True, "rows": len(processed)}
