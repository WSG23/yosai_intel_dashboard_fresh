...
async def explore_mapping_services(file_path: str, verbose: bool = False) -> dict:
    ...
    try:
        ...
        result = {"success": True, "file_info": {"rows": len(df), "columns": list(df.columns)}}
        ...
        try:
            ...
            if isinstance(learned_mappings, dict):
                logger.info(f"Learned mappings keys: {list(learned_mappings.keys())[:5]}")
                result["learned_mappings"] = {"count": len(learned_mappings), "keys": list(learned_mappings.keys())[:5]}
            else:
                result["learned_mappings"] = {"type": str(type(learned_mappings)), "value": str(learned_mappings)[:200]}
        ...
        try:
            ...
            if isinstance(user_device_mappings, dict):
                ...
                result["user_device_mappings"] = {"type": str(type(user_device_mappings)), "value": str(user_device_mappings)[:200]}
        except Exception as e:
