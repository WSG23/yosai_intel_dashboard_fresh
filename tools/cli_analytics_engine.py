...
async def test_analytics_with_mappings(verbose: bool = False) -> dict:
    try:
        ...
        if parquet_path.exists():
            ...
            enhanced_mappings = [
                (key, value) for key, value in learned_mappings.items() 
            ...
            if enhanced_mappings:
                ...
                result = {
                    ...
                    "data_info": {
                        ...
                        "sample_doors": list(df['Door Location'].unique())[:5] if 'Door Location' in df.columns else []
                    },
                    "mapping_info": {
                ...
                try:
                    ...
                    analytics_methods = [method for method in dir(analytics_service) if not method.startswith('_')]
                    result["analytics_service"] = {
                        "available_methods": analytics_methods,
                        "health_check": analytics_service.health_check() if hasattr(analytics_service, 'health_check') else "No health_check method"
                    }
                    print(f"AnalyticsService methods: {analytics_methods[:10]}...")  # Show first 10
                ...
                try:
                    ...
                    # Test available methods
                    upload_methods = [method for method in dir(upload_processor) if not method.startswith('_')]
                    result["upload_analytics"] = {
                        "available_methods": upload_methods
                    }
                    print(f"UploadAnalyticsProcessor methods: {upload_methods[:10]}...")  # Show first 10
                    ...
                    if hasattr(upload_processor, 'process_dataframe'):
                        try:
                            ...
                            result["upload_analytics"]["analysis_result"] = analysis_result
                            print(f"DataFrame analysis completed: {type(analysis_result)}")
                        except Exception as e:
                ...
                return {"success": False, "error": "No Enhanced Security Demo mappings found"}
        else:
            return {"success": False, "error": "Enhanced Security Demo parquet file not found"}
