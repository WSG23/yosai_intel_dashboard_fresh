...
def test_stages(test_df):
    ...
    try:
        ...
        if len(cleaned_df) != len(test_df):
            print(f"     TRUNCATION DETECTED in unicode processing!")
            return cleaned_df
    except Exception as e:
    ...
    try:
        ...
        if len(validated_df) != len(cleaned_df):
            print(f"     TRUNCATION DETECTED in security validation!")
            return validated_df
    except Exception as e:
    ...
    try:
        ...
        if len(reloaded_df) != len(validated_df):
            print(f"     TRUNCATION DETECTED in CSV reload!")
            return reloaded_df
    ...
    try:
        ...
        if hasattr(dynamic_config, 'analytics'):
            if hasattr(dynamic_config.analytics, 'max_records_per_query'):
                print(f"   Max records per query: {dynamic_config.analytics.max_records_per_query:,}")
