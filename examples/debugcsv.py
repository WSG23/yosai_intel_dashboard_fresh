...
def test_csv_file_directly(csv_path: str):
    ...
    try:
        ...
        logger.info(
            f"✅ Direct pd.read_csv(): {len(df_direct)} rows × {len(df_direct.columns)} columns"
        )
...
def test_base64_encoding(csv_path: str):
    ...
    try:
        ...
        logger.info(
            f"✅ Pandas read from StringIO: {len(df)} rows × {len(df.columns)} columns"
        )
...
def test_file_validator():
    ...
    try:
        ...
        if df is not None:
            logger.info(
                f"✅ File validator result: {len(df)} rows × {len(df.columns)} columns"
            )
            if len(df) < 90:  # Should have ~100 rows
                logger.warning(f"⚠️ FILE VALIDATOR IS TRUNCATING DATA!")
        ...
        logger.warning("⚠️ Could not import file validator for testing")
        return 0
    except Exception as e:
    ...
    try:
        ...
        if result.get("success"):
            ...
            if rows < 20:  # Your file should have 2000+ rows
                logger.warning(f"⚠️ UPLOAD SERVICE IS TRUNCATING DATA!")
        ...
        logger.warning("⚠️ Could not import upload service for testing")
        return 0
    except Exception as e:
